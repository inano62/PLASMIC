...
class CsvUserServiceImpl extends AbstractCsvService implements CsvUserService
{
    /**
    ...
    {
        $csv = $this->convertContentToResource($content);
        ...
        while ($row = fgetcsv($csv)) {
            if (is_null($columnGroup)) {
                ...
                $positionIndex = $this->arraySearch(['役職名称'], $row);
                ...
                $genderIndex = $this->arraySearch(['性別'], $row);
                ...
                if (!is_null($columnGroup->employeeCodeIndex)) {
                    $row[$columnGroup->employeeCodeIndex] = $this->normalizeInt($row[$columnGroup->employeeCodeIndex]);
                }
                if (!is_null($columnGroup->previousEmployeeCodeIndex)) {
                    $row[$columnGroup->previousEmployeeCodeIndex] = $this->normalizeInt($row[$columnGroup->previousEmployeeCodeIndex]);
                }
                ...
                if (!is_null($columnGroup->bumonGroupCodeIndex)) {
                    $row[$columnGroup->bumonGroupCodeIndex] = $this->normalizeInt($row[$columnGroup->bumonGroupCodeIndex]);
                }
                ...
                if (!is_null($columnGroup->bumonCodeIndex)) {
                    $row[$columnGroup->bumonCodeIndex] = $this->normalizeInt($row[$columnGroup->bumonCodeIndex]);
                }
                ...
                if (!is_null($columnGroup->bushoCodeIndex)) {
                    $row[$columnGroup->bushoCodeIndex] = $this->normalizeInt($row[$columnGroup->bushoCodeIndex]);
                }
                ...
                if (!is_null($columnGroup->positionIndex)) {
                    $row[$columnGroup->positionIndex] = $this->normalizeString($row[$columnGroup->positionIndex]);
                }
                ...
                if (!is_null($columnGroup->honmuBumonGroupCodeIndex)) {
                    $row[$columnGroup->honmuBumonGroupCodeIndex] = $this->normalizeInt($row[$columnGroup->honmuBumonGroupCodeIndex]);
                } 
                ...
                if (!is_null($columnGroup->honmuBumonCodeIndex)) {
                    $row[$columnGroup->honmuBumonCodeIndex] = $this->normalizeInt($row[$columnGroup->honmuBumonCodeIndex]);
                }
                ...
                if (!is_null($columnGroup->honmuBushoCodeIndex)) {
                    $row[$columnGroup->honmuBushoCodeIndex] = $this->normalizeInt($row[$columnGroup->honmuBushoCodeIndex]);
                }
                ...
                if (!is_null($columnGroup->genderIndex)) {
                    $row[$columnGroup->genderIndex] = $this->normalizeGender($row[$columnGroup->genderIndex]);
                }
                $rows[] = $row;
            }
        }
     ...
     * @return ?int
     */
    private function normalizeGender(?string $value): ?int
    {
        switch ($value) {
