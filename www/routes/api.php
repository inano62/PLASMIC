...
Route::post('/auth/token', function (Request $r) {
    ...
    if (!Auth::attempt($cred)) return response()->json(['message' => 'Unauthorized'], 401);
    $token = $r->user()->createToken('admin')->plainTextToken;
...
Route::post('/login', function (Request $r) {
    ...
    // 既存トークンを掃除（任意）— "admin" という名前のPATを一旦削除
